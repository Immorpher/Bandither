...
----------------------------------------------
To install this shader, replace the existing "Doom64.kpf" file in the Doom 64 Remaster folder. It will run initially with the default settings and only in OpenGL rendering mode. Your anti-aliasing menu will now be a "Shader" menu which you can now select Bandither as one of the options.

To change the Bandither settings, either unzip the Doom64.kpf or open it in a program like Slade. Within the "progs" folder, edit the text of the "fxaa_fast.shader". You will find the following options:
